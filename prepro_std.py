...
logger = create_logger(
    __name__,
    to_disk=True,
    log_file='mt_dnn_data_proc_.log')
...
def parse_args():
    ...
    parser.add_argument('--input_dir', type=str, default='data_my/tsv')
    parser.add_argument('--output_dir', type=str, default='data_my/json')
...
def main(args):
    ...
    for task in task_defs.get_task_names():
        ...
        for split_name in task_def.split_names:
            file_path = os.path.join(input_dir, "%s_%s.tsv" % (task, split_name))
            if not os.path.exists(file_path):
                logger.warning("File %s doesnot exit")
                sys.exit(1)
            rows = load_data(file_path, task_def)
            dump_path = os.path.join(output_dir, "%s_%s.json" % (task, split_name))
...
if __name__ == '__main__':
    ...
    --model ./mt_dnn_models/bert-base-chinese \
    --max_seq_len 128 \
    --input_dir ./data_my/data42mmm_without_name_tsv \
    --output_dir ./data_my/data42mmm_without_name_json \
    --task_def ./experiments/custom/top_sub_task_def.yml 
    '''
